...
export default function PostPage(props: PostType) {
  ...
  const authorArray = frontMatter.author?.split(',')
    ...
    <Layout>
      ...
      <div className="container max-w-screen-xl mx-auto px-8 py-16 sm:px-16 xl:px-20">
        ...
        <header className="mb-16 max-w-5xl space-y-4">
          ...
          <h1 className="text-3xl text-slate-700 font-medium font-serif">{frontMatter.title}</h1>
          <div className="text-sm">
            <p>{frontMatter.date}</p>
          </div>
          {!frontMatter.hideAuthor && (
          ...
          <main className="col-span-full lg:col-span-8 space-y-8">
            {frontMatter.thumb && (
              <div className="relative mb-8 h-96 w-full overflow-auto rounded border">
                <Image
                  src={'/image/blog/' + frontMatter.thumb}
                  layout="fill"
                  objectFit="cover"
                  alt="thumb of blog post"
                />
              </div>
            )}
            ...
            <div className="py-16">
              ...
              <div className="mt-4 flex items-center space-x-4">
                <Link
                  passHref
                  href={`https://twitter.com/share?text=${frontMatter.title}&url=${process.env.NEXT_PUBLIC_URL}/blog/${post.slug}`}
                >
                ...
                <Link
                  passHref
                  href={`https://www.linkedin.com/shareArticle?url=${process.env.NEXT_PUBLIC_URL}/blog/${post.slug}&title=${frontMatter.title}`}
                >
                  <a target="_blank" className="text-slate-500 hover:text-slate-700">
                    <svg
                      width="20"
...
export const getStaticPaths: GetStaticPaths = async function () {
  const paths = await getAllPostPaths('_blog')
